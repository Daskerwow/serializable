...
// Map
// =============================================================================
/// Parses a single entry, attributing a failure to whichever of
/// [keyParser] / [valueParser] actually threw.
///
/// Previously both were reported as `key "$k": $e"`, which was misleading
/// whenever it was really the *value* that failed to parse.
@pragma('vm:prefer-inline')
MapEntry<K, V> _parseMapEntry<K, V>(
  String fnName,
  Object? k,
  Object? val,
  Parser<K> keyParser,
  ...
  final K parsedKey;
  try {
    parsedKey = keyParser(k);
  } catch (e, st) {
    Error.throwWithStackTrace(
      FormatException('$fnName<$K,$V> key "$k": $e'),
      st,
    );
  }
  try {
    return MapEntry(parsedKey, valueParser(val));
  } catch (e, st) {
    Error.throwWithStackTrace(
      FormatException('$fnName<$K,$V> value at key "$k": $e'),
      st,
    );
  }
}
...
Parser<Map<K, V>> mapOf<K, V>(Parser<K> keyParser, Parser<V> valueParser) =>
    (Object? v) => switch (v) {
      final Map m => Map<K, V>.unmodifiable(
        m.map(
          (k, val) => _parseMapEntry('mapOf', k, val, keyParser, valueParser),
        ),
      ),
      _ => <K, V>{},
    };
  ...
  Parser<V> valueParser,
    ...
    (Object? v) => switch (v) {
      final Map m => Map<K, V>.unmodifiable(
        m.map(
          (k, val) =>
              _parseMapEntry('mapOrNullOf', k, val, keyParser, valueParser),
        ),
      ),
      _ => null,
    };
  ...
  Parser<V> valueParser,
    ...
    (Object? v) => switch (v) {
      final Map m => Map<K, V>.unmodifiable(
        m.map(
          (k, val) =>
              _parseMapEntry('mapOrThrowOf', k, val, keyParser, valueParser),
        ),
      ),
      _ => throw FormatException(
